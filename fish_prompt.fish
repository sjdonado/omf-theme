...
function fish_prompt
  ...
  if test "$theme_ignore_ssh_awareness" != 'yes' -a -n "$SSH_CLIENT$SSH_TTY"
    set prompt_string "$fish "(whoami)"@"(hostname -s)" $ending"
  end
  ...
  if git_is_repo
    ...
    echo -n -s " $git_s" $repository_color (git_branch_name) $normal_color " "
    ...
    echo -n -s " " $directory_color $cwd " " $normal_color $ending
  end
end
